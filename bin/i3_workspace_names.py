...
## Source: https://gitlab.com/flib99/i3-workspace-names
...
def rename(i3, e):
    for i in i3.get_tree().workspaces():
        ...
        for j in i.leaves():
            ...
            icon = replace(j.name, icon)

            windows += icon + ' '
            # windows += icon + ' ' + replace(j.name) + ' '
            # windows += icon + ' ' + str(j.name)
        # print('rename workspace "%s" to "%s: %s"' %
